...
const baseURL = (process.env.BETTER_AUTH_URL || process.env.NEXT_PUBLIC_APP_URL || process.env.VERCEL_PROJECT_PRODUCTION_URL || process.env.VERCEL_URL || 'http://localhost:3000').replace(/\/$/, '')
...
export async function getAuth() {
  if (!authPromise) {
    authPromise = getDb().then((db) => betterAuth({
      ...
      socialProviders: {
        google: { clientId: process.env.GOOGLE_CLIENT_ID || 'demo', clientSecret: process.env.GOOGLE_CLIENT_SECRET || 'demo' },
        github: { clientId: process.env.GITHUB_CLIENT_ID || 'demo', clientSecret: process.env.GITHUB_CLIENT_SECRET || 'demo' },
      },
      user: {
        additionalFields: {
