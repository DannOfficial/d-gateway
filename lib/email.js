...
export async function sendGmailVerificationEmail({ to, name, verificationUrl }) {
  ...
  const port = 587
  const user = 'gentursy02@gmail.com'
  const pass = 'grbccbqwwckcyxxn'
  const from = process.env.GMAIL_FROM || process.env.SMTP_FROM || (user ? `Dann-Tele Gateway <${user}>` : 'Dann-Tele Gateway')
